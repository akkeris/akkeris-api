...
function akkeris(path, headers, method, body, user, cb) {
  ...
  if (!user.proxyAuth) {
    headers = { Authorization: process.env.CONTROLLER_API_TOKEN, ...(headers || {}) };
  }
  ...
  const connector = process.env.CONTROLLER_API_URL.startsWith('http://') ? http : https;
  const req = connector.request(parsedURL, options, (response) => {
